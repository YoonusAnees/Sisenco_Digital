  ...
  return (
    ...
    >
      <div className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
        {/* Top bar with status & edit action */}
        ...
        {report.lastCorrectionNote && (
          <div className="flex gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div>
              <p className="text-xs font-semibold text-amber-700 mb-0.5">Manager Correction Request</p>
        ...
        {report.completedTasks?.length > 0 && (
          <SectionCard
            title={`Completed Tasks (${report.completedTasks.length})`}
        ...
        {report.nextWeekTasks?.length > 0 && (
          <SectionCard
            title={`Next Week Plan (${report.nextWeekTasks.length})`}
            ...
            <div className="space-y-3">
              {report.blockers.map((b, i) => (
                <div key={b._id || i} className="p-3 bg-red-50 rounded-lg border border-red-100">
                  ...
                  {b.assistanceNeeded && (
                    <p className="text-xs text-red-500 mt-1">
                      <span className="font-medium">Needs:</span> {b.assistanceNeeded}
                    </p>
                  )}
        ...
        {report.achievements?.length > 0 && (
          <SectionCard
            title={`Achievements (${report.achievements.length})`}
                ...
                >
                  <div>
                    ...
                    <span className="text-xs text-slate-400 ml-2">
                      · {HOURS_CATEGORY_LABELS[h.category] || h.category}
                    </span>
