...
const hoursBreakdownSchema = z.object({
  project: z.string().min(1, 'Project required'),
  category: z.enum([
    'development', 'testing', 'design', 'meetings',
    'research', 'documentation', 'support', 'other',
  ]),
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5 max-h-[75vh] overflow-y-auto pr-1">
          ...
          {completedTasksField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
              <div className="sm:col-span-4">
                <Input placeholder="Task title *" {...register(`completedTasks.${idx}.title`)} error={(errors.completedTasks?.[idx] as any)?.title?.message} />
              </div>
              <div className="sm:col-span-4">
                <Select options={projectOptions} {...register(`completedTasks.${idx}.project`)} error={(errors.completedTasks?.[idx] as any)?.project?.message} />
              </div>
              <div className="sm:col-span-2">
                <Input type="number" placeholder="Hrs" step="0.5" min="0" max="24" {...register(`completedTasks.${idx}.hoursSpent`)} error={(errors.completedTasks?.[idx] as any)?.hoursSpent?.message} />
              </div>
              <div className="sm:col-span-2 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500 w-full" onClick={() => completedTasksField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
              <div className="sm:col-span-12">
                <Input placeholder="Description (optional)" {...register(`completedTasks.${idx}.description`)} />
              </div>
            </div>
          ))}
          <Button type="button" variant="ghost" size="sm" leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => completedTasksField.append({ title: '', description: '', project: '', hoursSpent: 0 })}>
            Add Task
          </Button>
        </Section>
        ...
        <Section title="Next Week Plan" count={nextWeekTasksField.fields.length} accent="#B7872A">
          {nextWeekTasksField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
              <div className="sm:col-span-4">
                <Input placeholder="Task title *" {...register(`nextWeekTasks.${idx}.title`)} error={(errors.nextWeekTasks?.[idx] as any)?.title?.message} />
              </div>
              <div className="sm:col-span-3">
                <Select options={projectOptions} {...register(`nextWeekTasks.${idx}.project`)} error={(errors.nextWeekTasks?.[idx] as any)?.project?.message} />
              </div>
              <div className="sm:col-span-2">
                <Select options={priorityOptions} {...register(`nextWeekTasks.${idx}.priority`)} />
              </div>
              <div className="sm:col-span-2">
                <Input type="date" {...register(`nextWeekTasks.${idx}.dueDate`)} />
              </div>
              <div className="sm:col-span-1 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500" onClick={() => nextWeekTasksField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
            </div>
          ...
          <Button type="button" variant="ghost" size="sm" leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => nextWeekTasksField.append({ title: '', project: '', priority: 'medium', dueDate: null })}>
            Add Planned Task
          </Button>
        </Section>
          ...
          {blockersField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 bg-red-50 rounded-xl border border-red-100">
              <div className="sm:col-span-5">
                <Input placeholder="Blocker title *" {...register(`blockers.${idx}.title`)} error={(errors.blockers?.[idx] as any)?.title?.message} />
              </div>
              <div className="sm:col-span-5">
                <Select options={projectOptions} {...register(`blockers.${idx}.project`)} error={(errors.blockers?.[idx] as any)?.project?.message} />
              </div>
              <div className="sm:col-span-2 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500 w-full" onClick={() => blockersField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
              <div className="sm:col-span-12">
                <Input placeholder="Description *" {...register(`blockers.${idx}.description`)} error={(errors.blockers?.[idx] as any)?.description?.message} />
              </div>
              <div className="sm:col-span-6">
                <Input placeholder="Impact..." {...register(`blockers.${idx}.impact`)} />
              </div>
              <div className="sm:col-span-6">
                <Input placeholder="Assistance needed..." {...register(`blockers.${idx}.assistanceNeeded`)} />
              </div>
            </div>
          ))}
          <Button type="button" variant="ghost" size="sm" leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => blockersField.append({ title: '', description: '', project: '', impact: '', assistanceNeeded: '' })}>
            Add Blocker
          </Button>
        </Section>
        ...
        <Section title="Achievements" count={achievementsField.fields.length} accent="#059669">
          {achievementsField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 bg-green-50 rounded-xl border border-green-100">
              <div className="sm:col-span-5">
                <Input placeholder="Achievement title *" {...register(`achievements.${idx}.title`)} error={(errors.achievements?.[idx] as any)?.title?.message} />
              </div>
              <div className="sm:col-span-5">
                <Select options={[{ value: '', label: 'No project (general)' }, ...projectOptions.slice(1)]} {...register(`achievements.${idx}.project`)} />
              </div>
              <div className="sm:col-span-2 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500 w-full" onClick={() => achievementsField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
              <div className="sm:col-span-12">
                <Input placeholder="Description (optional)" {...register(`achievements.${idx}.description`)} />
              </div>
            </div>
          ))}
          <Button type="button" variant="ghost" size="sm" leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => achievementsField.append({ title: '', description: '', project: null })}>
            Add Achievement
          </Button>
        </Section>
        ...
        <Section title="Hours Breakdown" count={hoursBreakdownField.fields.length} accent="#7C3AED">
          {hoursBreakdownField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 p-3 bg-violet-50 rounded-xl border border-violet-100">
              <div className="sm:col-span-4">
                <Select options={projectOptions} {...register(`hoursBreakdown.${idx}.project`)} error={(errors.hoursBreakdown?.[idx] as any)?.project?.message} />
              </div>
              <div className="sm:col-span-3">
                <Select options={categoryOptions} {...register(`hoursBreakdown.${idx}.category`)} />
              </div>
              <div className="sm:col-span-2">
                <Input type="number" placeholder="Hrs" step="0.5" min="0.25" {...register(`hoursBreakdown.${idx}.hours`)} error={(errors.hoursBreakdown?.[idx] as any)?.hours?.message} />
              </div>
              <div className="sm:col-span-2">
                <Input placeholder="Notes..." {...register(`hoursBreakdown.${idx}.notes`)} />
              </div>
              <div className="sm:col-span-1 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500" onClick={() => hoursBreakdownField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
            </div>
          ...
          <Button type="button" variant="ghost" size="sm" leftIcon={<Plus className="w-4 h-4" />}
            onClick={() => hoursBreakdownField.append({ project: '', category: 'development', hours: 0, notes: '' })}>
            Add Hours Entry
          </Button>
        </Section>
        ...
        <Section title="Supporting Links" count={linksField.fields.length} accent="#0891B2">
          {linksField.fields.map((field, idx) => (
            <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2">
              <div className="sm:col-span-3">
                <Input placeholder="Label *" {...register(`links.${idx}.label`)} error={(errors.links?.[idx] as any)?.label?.message} />
              </div>
              <div className="sm:col-span-8">
                <Input placeholder="https://..." {...register(`links.${idx}.url`)} error={(errors.links?.[idx] as any)?.url?.message} />
              </div>
              <div className="sm:col-span-1 flex items-center">
                <Button type="button" variant="ghost" size="sm" className="text-red-500" onClick={() => linksField.remove(idx)}>
                  <Trash2 className="w-4 h-4" />
                </Button>
              </div>
            </div>
